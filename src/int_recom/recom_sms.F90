subroutine REcoM_sms(n,Nn,state,thick,recipthick,SurfSR,sms,Temp, Sali_depth &
    ...
    Real(kind=8),dimension(mesh%nl-1),intent(inout)         :: OmegaA_watercolumn   !< aragonite saturation state
    Real(kind=8),dimension(mesh%nl-1),intent(inout)         :: kspc_watercolumn     !< stoichiometric solubility product for calcite [mol^2/kg^2]
    Real(kind=8),dimension(mesh%nl-1),intent(inout)         :: kspa_watercolumn     !< stoichiometric solubility product for aragonite [mol^2/kg^2]
...
#include "../associate_mesh.h"
    ...
    tiny_N_p = tiny_chl/chl2N_max_p    ! Phaeocystis
...
!       do k=1, Nn
            ...
            PhaeoN   = max(tiny_N_p,state(k,iphan)   + sms(k,iphan ))  ! Phaeocystis
...
! For Mocsy
            ...
            if (calc_zoo) then   ! NEW CALC_ZOO
               HetAra = max(tiny,state(k,ihetara)    + sms(k,ihetara))
               MicCal = max(tiny,state(k,imiccal)    + sms(k,imiccal))
               DetZ2Ara = max(tiny,state(k,idetz2ara)+ sms(k,idetz2ara))
            endif
...
!< Quotas
            ...
            quota_phaeo       = PhaeoN / PhaeoC         ! Phaeocystis
...
!< New phytoplankton temperature functions
           ...
           !Temp_cocco = max(0.1419d0 * Temp(k)**0.8151d0,tiny) ! Function from Fielding 2013; is based on observational GR, but range fits best to ours
...
! Calculate the carbonate system for the very first time step of the first year of the run
    !if (mocsy_restart==.false. .and. recom_istep==1) then    ! r_restart is defined in gen_modules_clock in fesom_cpl.
            dpos(1) = -zF(k)
            if (mstep==1) then
                call vars_sprac(ph_depth, pco2_depth, fco2_depth, co2_depth, hco3_depth, co3_depth, OmegaA_depth, OmegaC_depth, kspc_depth, kspa_depth, BetaD_depth,  &
                ...
                OmegaA_watercolumn(k) = OmegaA_depth(1) ! NEW CALC_ZOO
                kspc_watercolumn(k)   = kspc_depth(1)
                kspa_watercolumn(k)   = kspa_depth(1)   ! NEW CALC_ZOO
...
!------------------------------------------------------------------------------
            ...
            if (OmegaC_diss) then    ! Calcdiss dependent on carbonate saturation
                ...
                CO3_sat_ara = (kspa_watercolumn(k) / Ca) * rhoSW_watercolumn(k) ! Saturated carbonate ion concentration, converted to [mol/m3] ! NEW CALC_ZOO
                if (Aumont_2015) then
                ...
                if (Naviaux_2019) then  ! NEW CALC_ZOO
                   ...
                   if (omegac_caco3 >= diss_threshold) then  ! low undersaturation
                ...
                if (calc_zoo) then ! NEW CALC_ZOO
                   if (Aumont_2015) then
                      calc_diss_ara = calc_diss_omegac * max(zero,(1-(CO3_watercolumn(k)/CO3_sat_ara)))**(calc_diss_exp) ! REPLACE WITH ARAGONITE VALUES!!
                   endif
                   if (Naviaux_2019) then
                      omegaa_caco3 = CO3_watercolumn(k)/CO3_sat_ara
                      if (omegaa_caco3 >= diss_threshold) then	 ! low undersaturation
                         calc_diss_ara = ara_diss_fac_low *  max(zero,(1-(CO3_watercolumn(k)/CO3_sat_ara)))**(ara_diss_exp_low)
                      else                               ! high undersaturation
                         calc_diss_ara = ara_diss_fac_high *  max(zero,(1-(CO3_watercolumn(k)/CO3_sat_ara)))**(ara_diss_exp_high)
                      endif
                   endif
...
!< ************************
            ...
            !pMax_cocco =  P_cm_c * qlimitFac * Temp_cocco ! USE THIS LINE WHEN USING OLD COCCO TEMP FUNCTION
...
!< **********************************
                ...
                if (KOchl_phaeo /= KOchl_phaeo) then
                    print*,' KOchl_phaeo is ', KOchl_phaeo
                    print*,' deg_Chl_p is ', deg_Chl_p
                    print*,' alfa_p is ', alfa_p
                    print*,' CHL2C_p is ', CHL2C_plast_phaeo
                    print*,' PARave is ', PARave
                    print*,' pMax_p is ', pMax_phaeo
                    stop
                end if 
#endif
            end if ! photodamage 
...
!! use energy on silicon uptake.
            ...
            V_cm            = V_cm_fact_p                            ! Phaeocystis
            limitFacN_phaeo = recom_limiter(NMaxSlope, quota_phaeo, NCmax_p)
            N_assim_phaeo   = V_cm * pMax_phaeo * NCUptakeRatio_p &
                               * limitFacN_phaeo * DIN/(DIN + k_din_p)     
#endif
...
!< biosynth: The cost of biosynthesis of N [mmol C mmol N−1 ]
            ...
            phyRespRate_phaeo = res_phy_p * limitFacN_phaeo + biosynth * N_assim_phaeo ! Phaeocystis
#endif
...
!< pzMicZoo: Maximum microzooplankton preference
            ...
            if (REcoM_Grazing_Variable_Preference) then   ! CHECK ONUR
                ...
                aux = aux + pzPhaeo*PhaeoN     ! Phaeocystis
#endif
! ******************************************************************************
                ...
                varpzPhaeo = (pzPhaeo*PhaeoN)/aux     ! Phaeocystis
#endif
! ******************************************************************************
                fDiaN = varpzDia * DiaN
                fPhyN = varpzPhy * PhyN
                ...
                fPhaeoN = varpzPhaeo * PhaeoN     ! Phaeocystis
#endif
                ...
                fPhaeoN = pzPhaeo * PhaeoN     ! Phaeocystis
#endif
...
! ******************************************************************************
            ...
            grazingFlux_Phaeo = grazingFlux * fPhaeoN / food     ! Phaeocystis
#endif
...
!< **************************
            ...
            grazingFluxcarbon_mes = grazingFluxcarbon_mes                              &     ! Phaeocystis
...
!< pzMicZoo2: Maximum microzooplankton preference
            ...
            if (REcoM_Grazing_Variable_Preference) then
                ...
                aux = aux + pzPhaeo2 * PhaeoN     ! Phaeocystis
#endif
! ******************************************************************************
                ...
                varpzPhaeo2 = (pzPhaeo2 * PhaeoN)/aux     ! Phaeocystis
#endif 
...
! ******************************************************************************
                ...
                fPhaeoN2 = varpzPhaeo2 * PhaeoN     ! Phaeocystis
#endif
                fHetN = varpzHet * HetN
                if (Grazing_detritus) then
                ...
                fPhaeoN2 = pzPhaeo2 * PhaeoN     ! Phaeocystis
#endif
                fHetN = pzHet * HetN
                if (Grazing_detritus) then
                    fDetN2 = pzDet2 * DetN
...
!< **********************
            ...
            food2 = food2 + fPhaeoN2     ! Phaeocystis
#endif
! ******************************************************************************
            foodsq2 = food2**2
            ...
            grazingFlux_Phaeo2 = (grazingFlux2 * fPhaeoN2)/food2     ! Phaeocystis
#endif
            ...
            grazingFluxcarbonzoo2 = grazingFluxcarbonzoo2 +                                &  ! Phaeocystis
...
!< pzCocco3: Maximum coccolithophore preference
            ...
            if (REcoM_Grazing_Variable_Preference) then
                ...
                aux = aux + pzPhaeo3 * PhaeoN     ! Phaeocystis
#endif
! ******************************************************************************
                ...
                varpzPhaeo3 = (pzPhaeo3 * PhaeoN)/aux    ! Phaeocystis
#endif
! ******************************************************************************
                ...
                fPhaeoN3 = varpzPhaeo3 * PhaeoN     ! Phaeocystis
#endif
            else ! REcoM_Grazing_Variable_Preference = .false.
                ...
                fCoccoN3 = pzCocco3 * CoccoN
                 fPhaeoN3 = pzPhaeo3 * PhaeoN     ! Phaeocystis
#endif
...
!< **********************
            ...
            food3 = food3 + fPhaeoN3     ! Phaeocystis
#endif
! ******************************************************************************
            foodsq3 = food3**2
            ...
            grazingFlux_Phaeo3 = (grazingFlux3 * fPhaeoN3)/food3     ! Phaeocystis
#endif
#endif
...
!-------------------------------------------------------------------------------
            ...
            aggregationrate = aggregationrate + agg_PP * PhaeoN  ! Phaeocystis
#endif
...
! Without coccolithophores, calcification is performed by a fraction of small phytoplankton
            ...
            calcification = 1.0d0 * Cphot_cocco * CoccoC * PICPOCtemp * PICPOCN
...
!< *** Microzooplankton calcification (calcite precipitation by forams) ***   ! NEW CALC_ZOO
!< ************************************************************************
...
!< ****************************************************************************
          ...
          ! Grazing
             ...
             ! Mortality
#if defined (__3Zoo2Det)
             hetLossFlux_ara = hetLossFlux * (recipQZoo/(HetC + tiny) * (HetAra * (HetC / (HetC + Zoo2C)))) ! instead of multiplying with recipquota, this scales to C:CaCO3 ratio; * (HetC / (HetC + Zoo2C)) computes the share of het in the total ara pool (possible because of constant PIC:POC ratio) 
#endif


             
!< *** Macrozoplankton calcification (aragonite precipitation by polar pteropods) *** ! NEW CALC_ZOO
!< ****************************************************************************
          ! Calcification                                                         
          if (Grazing_detritus) then
#if defined (__3Zoo2Det)
             calcification_macara = ( &
                  pic_poc_ptero_polar * calc_prod_ratio_macro &
          ...
          ! Mortality
#if defined (__3Zoo2Det)
          macLossFlux_ara = Zoo2LossFlux * (recipQZoo2/(Zoo2C + tiny) * (Zoo2C * (Zoo2C / (HetC + Zoo2C)))) ! instead of multiplying with recipquota, this scales to C:CaCO3 ratio; * (Zoo2C / (HetC + Zoo2C)) computes the share of het in the total ara pool (possible because of constant PIC:POC ratio)
#endif
...
!! Schourup 2013 Eq. A2
    ...
    sms(k,idin)      = (                     &
        ...
        - N_assim_phaeo            * PhaeoC  &  ! --> N assimilation Phaeocystis
#endif
...
!< rho_C1: Temperature dependent C degradation of extracellular organic C (EOC) [day^-1]
    ...
    sms(k,idic)      = (                                         &
        ...
        - Cphot_Phaeo                     * PhaeoC               & ! --> Phaeocystis photosynthesis
        + phyRespRate_Phaeo               * PhaeoC               & ! --> Phaeocystis respiration
#endif
        ...
        + calc_loss_gra    * grazEff      * calc_diss_guts_meso  & ! --> Additional dissolution in mesozooplankton guts ! NEW CALC_ZOO added _meso and grazEff
#if defined (__3Zoo2Det)
        ...
        - calcification_macara                                   & ! --> Calcification of macrozooplankton (polar pteropods -> aragonite) ! NEW CALC_ZOO
#endif
...
!< N_assimC: 1.0625 = 1/16 + 1
    ...
    sms(k,ialk)      = (                                            &
        ...
        + 1.0625 * N_assim_Phaeo             * PhaeoC               & ! Phaeocystis
#endif
        ...
        + 2.d0 * miccal_loss_gra2 * grazEff2 * calc_diss_guts_macro & ! NEW CALC_ZOO
        + 2.d0 * hetara_loss_gra2 * grazEff2 * ara_diss_guts        & ! NEW CALC_ZOO
        + 2.d0 * calc_diss2                  * DetZ2Calc            &
        + 2.d0 * calc_diss_ara               * DetZ2Ara             & ! NEW CALC_ZOO
        - 2.d0 * calcification_miccal                               & ! NEW CALC_ZOO
        - 2.d0 * calcification_hetara                               & ! NEW CALC_ZOO
        - 2.d0 * calcification_macara                               & ! NEW CALC_ZOO
#endif
        - 2.d0 * calcification                                      &
                                                          ) * dt_b + sms(k,ialk)
    else
...
! Detritus N
    if (Grazing_detritus) then
        ...
        sms(k,idetn)       = (                             &
            ...
            + grazingFlux_Phaeo3 *                         & ! Phaeocystis
            - grazingFlux_Phaeo3 * grazEff3                &
	    ...
	    + grazingFlux_phy                              & ! Technically it is mesooooooooooooooooo
            ...
            + aggregationRate               * CoccoN       &
            + grazingFlux_Phaeo *                          & ! Phaeocystis
            - grazingFlux_Phaeo * grazEff                  &
            + aggregationRate               * PhaeoN       &
#endif
...
! Detritus C
    if (Grazing_detritus) then
        ...
        sms(k,idetc)       = (                                 &
            ...
            + grazingFlux_Phaeo3 * recipQuota_Phaeo            & ! Phaeocystis
            - grazingFlux_Phaeo3 * recipQuota_Phaeo * grazEff3 &
        ...
        sms(k,idetc)       = (                                 &
            ...
            - grazingFlux_Cocco * recipQuota_Cocco * grazEff   &
            + aggregationRate                      * CoccoC    &
            + grazingFlux_Phaeo * recipQuota_Phaeo             & ! Phaeocystis
            - grazingFlux_Phaeo * recipQuota_Phaeo * grazEff   &
            + aggregationRate                      * PhaeoC    &
#endif
            - grazingFlux_Det  * recipDet  * grazEff           &
...
!< Second Zooplankton Detritus calcite
    if (calc_zoo) then  ! NEW CALC_ZOO
      sms(k,idetz2calc)   = (                                  &
          + calc_loss_gra2                                     &
          - calc_loss_gra2   * grazEff2 * calc_diss_guts_macro & ! NEW CALC_ZOO added _macro and grazEff2
          + calc_loss_gra                                      &
          - calc_loss_gra    * grazEff  * calc_diss_guts_meso  & ! NEW CALC_ZOO added _meso and grazEff
          + miccal_loss_gra                                    &
          - miccal_loss_gra  * grazEff  * calc_diss_guts_meso  & ! NEW CALC_ZOO
          + miccal_loss_gra2                                   &
          - miccal_loss_gra2 * grazEff2 * calc_diss_guts_macro & ! NEW CALC_ZOO
...
!< DON (Extracellular organic N)
    ...
    sms(k,idon)      = (                      &
        ...
        + lossN_p * limitFacN_Phaeo * PhaeoN  & ! Phaeocystis
#endif
...
!< EOC
    ...
    sms(k,idoc)       = (                     &
        ...
        + lossC_p * limitFacN_Phaeo * PhaeoC  & ! Phaeocystis
#endif
...
!< lossN_p:  Phaeocystis loss of organic N compounds [day^-1]

    sms(k,iphan)      = (                                  &
        + N_assim_phaeo                          * PhaeoC  & ! --> N assimilation
        - lossN_p * limitFacN_phaeo              * PhaeoN  & ! --> DON excretion
        - aggregationRate                        * PhaeoN  & ! --> Aggregation loss
...
!< lossC_p: Phaeocystis loss of carbon [day^-1]
!< When N : C ratio becomes too high, excretion of DOC is downregulated
!< by the limiter function limitFacN
!< aggregationRate transfers C to the detritus pool

    sms(k,iphac)      = (                                  &
        + Cphot_phaeo                            * PhaeoC  & ! --> Photosynthesis ---->/
        - lossC_p * limitFacN_phaeo              * PhaeoC  & ! --> Excretion of DOC   / Net photosynthesis
        - phyRespRate_phaeo                      * PhaeoC  & ! --> Respiration ----->/
        - aggregationRate                        * PhaeoC  & ! --> Aggregation loss          
        - grazingFlux_phaeo  * recipQuota_phaeo            & ! --> Grazing loss
#if defined (__3Zoo2Det)
        - grazingFlux_phaeo2 * recipQuota_phaeo            &
...
!< Chl2N = PhyChl/PhyN
    ...
    sms(k,iphachl)       = (                               &
        + chlSynth_phaeo                       * PhaeoC    & ! --> Chl-a synthesis
        - KOchl_phaeo                          * PhaeoChl  & ! --> Degradation loss
        - aggregationRate                      * PhaeoChl  & ! --> Aggregation loss
        - grazingFlux_phaeo  * Chl2N_phaeo                 & ! --> Grazing loss
#if defined (__3Zoo2Det)
...
!< kScavFe: Scavenging rate of iron [m3 mmol C^-1 day^-1]
    ...
    sms(k,ife) = ( Fe2N * (                         &
        ...
        - N_assim_cocco                   * CoccoC  & 
        + lossN_c       * limitFacN_cocco * CoccoN  & 
        - N_assim_phaeo                   * PhaeoC  & ! Phaeocystis
...
!< Small phytoplankton calcite
    ...
    sms(k,iphycal)    = (                               &
        ...
        !- phyRespRate_cocco                   * PhyCalc & ! --> Respiration   -- NEW CALCZOO Nov25 take this out as not a real loss
...
! Detritus calcite
    ...
    if (calc_zoo) then ! NEW CALC_ZOO
      sms(k,idetcal)   = (                                           &
        ...
        - calc_loss_gra3    * grazEff3        * calc_diss_guts_micro &  ! NEW CALC_ZOO added _micro and grazEff3
        - calc_diss                           * DetCalc              &
                                                       ) * dt_b + sms(k,idetcal)
    else
      sms(k,idetcal)   = (                                             &
...
! Calcifying zooplankton    ! NEW CALC_ZOO    
    if (calc_zoo) then
       ...
       sms(k,ihetara) = (                         &     
          + calcification_hetara                  &
          + calcification_macara                  & ! NEW added this to account for polar pteropods; remaining fluxes (grazing, detritus) stay fixed to meso
          - hetara_loss_gra2                      & ! goes to 2nd det aragonite & DIC & Alk
          - hetLossFlux_ara                       & ! goes to 2nd det aragonite
          - macLossFlux_ara                       & ! goes to 2nd det aragonite
       ...
       sms(k,idetz2ara) = (                               &
            ...
            + hetara_loss_gra2                            &
            - hetara_loss_gra2 * grazEff2 * ara_diss_guts &
...
! Oxygen
    ...
    sms(k,ioxy)   = (                       &
      ...
      + Cphot_cocco                * CoccoC &
      - phyRespRate_cocco          * CoccoC &
      + Cphot_phaeo                * PhaeoC & ! Phaeocystis
...
! DIC_13
      sms(k,idic_13) =        (                                &
            ...
            + calc_loss_gra_13 * grazEff * calc_diss_guts_meso & ! NEW CALC_ZOO added _meso and grazEff
            - calcification_13                                 &
                                ) * dt_b + sms(k,idic_13)
!-------------------------------------------------------------------------------
! Phytoplankton C_13
...
! DIC_14
          sms(k,idic_14) =        (                            &
            ...
            + calc_loss_gra_14 * grazEff * calc_diss_guts_meso & ! NEW CALC_ZOO added _meso and grazEff
            - calcification_14                                 &
                                ) * dt_b + sms(k,idic_14)
!-------------------------------------------------------------------------------
! Phytoplankton C_14
...
!*** Net primary production [mmol C /(m3 * day)]
        ...
        vertNPPp(k) = vertNPPp(k)  + (   &    ! Phaeocystis
        + Cphot_phaeo             * PhaeoC  &
        - PhyRespRate_phaeo       * PhaeoC  &
        ) * recipbiostep
#endif
...
!*** Gross primary production [mmol C /(m3 * day)]
        ...
        vertGPPp(k) = vertGPPp(k)  + (   &    ! Phaeocystis
        + Cphot_phaeo             * PhaeoC  &
        ) * recipbiostep
#endif
...
!*** Net N-assimilation [mmol N/(m3 * day)]
        ...
        vertNNAp(k) = vertNNAp(k)  + (   &    ! Phaeocystis
...
!*** Changed to chlorophyll degradation (commented out gross N-assimilation below)
        ...
        vertChldegp(k) = vertChldegp(k)  + (   & ! Phaeocystis
        + KOchl_phaeo &
        ) * recipbiostep
#endif
...
!*** Grazing on phaeocystis by First Zooplankton (without grazeff, i.e. loss term for Phaeo)
...
!*** Grazing on Phaeocystis by second zooplankton (without grazeff, i.e. loss term for PHAEO)
...
!*** Grazing on Phaeocystis by third zooplankton (without grazeff, i.e. loss term for PHAEO)
...
!*** calc_diss_guts                        ! NEW CALC_ZOO
        ...
        + calc_loss_gra    * grazEff * calc_diss_guts_meso        &
        + miccal_loss_gra  * grazEff * calc_diss_guts_meso        &
        ) * recipbiostep
        
        vertcalcdiss_guts_macro(k) = vertcalcdiss_guts_macro(k) + ( &
        + calc_loss_gra2   * grazEff2 * calc_diss_guts_macro        &
        + miccal_loss_gra2 * grazEff2 * calc_diss_guts_macro        &
        ) * recipbiostep
...
!***    aggregation by phaeocystis
        vertaggp(k) = vertaggp(k) + (             & ! Phaeocystis
        + aggregationrate * PhaeoC                &
        ) * recipbiostep
#endif                                
...
!*** excretion of DOC by phaeocystis
        vertdocexp(k) = vertdocexp(k) + (         & ! Phaeocystis
...
!*** calcification
        ...
        if (calc_zoo) then
           ...
           + lossC_c * limitFacN_cocco * PhyCalc       &
           !+ phyRespRate_cocco * PhyCalc               & -> outcommented because not a real loss (Nov25)
           + calc_loss_agg                             &
           + calc_loss_gra                             &
           + calc_loss_gra2                            &
           + calc_loss_gra3                            & 
           ...
           verthetmort_ara(k) = verthetmort_ara(k) + ( &
           + hetLossFlux_ara                           &
           ) * recipbiostep

           vertmacmort_ara(k) = vertmacmort_ara(k) + ( &
           + macLossFlux_ara                           &
           ) * recipbiostep

           vertmicmort_cal(k) = vertmicmort_cal(k) + ( &
           + MicZooLossFlux_cal                        &
           ) * recipbiostep

           verthetgra2_ara(k) = verthetgra2_ara(k) + ( &
           + hetara_loss_gra2                          &
           ) * recipbiostep

           vertmicgra_cal(k) = vertmicgra_cal(k) + ( &
           + miccal_loss_gra                         &
           ) * recipbiostep

           vertmicgra2_cal(k) = vertmicgra2_cal(k) + ( &
           + miccal_loss_gra2                          &
           ) * recipbiostep

           vertcocco_excr_cal(k) = vertcocco_excr_cal(k) + ( &
           + lossC_c * limitFacN_cocco * PhyCalc             &
           ) * recipbiostep

           !vertcocco_resp_cal(k) = vertcocco_resp_cal(k) + ( &  ! Nov25 outcommented because not a real loss term of calc
           !+ phyRespRate_cocco * PhyCalc                     &
           !) * recipbiostep

           vertcocco_agg_cal(k) = vertcocco_agg_cal(k) + ( &
           + calc_loss_agg                                 &
           ) * recipbiostep

           vertcoccogra_cal(k) = vertcoccogra_cal(k) + ( &
           + calc_loss_gra                               & 
           ) * recipbiostep

           vertcoccogra2_cal(k) = vertcoccogra2_cal(k) + ( &
           + calc_loss_gra2                                &
           ) * recipbiostep

           vertcoccogra3_cal(k) = vertcoccogra3_cal(k) + ( &
           + calc_loss_gra3                                &
           ) * recipbiostep
...
! phaeo resipration
        vertrespp(k) = vertrespp(k) + (           & ! Phaeocystis
        + PhyRespRate_phaeo       * PhaeoC        &
        ) * recipbiostep
#endif
endif
